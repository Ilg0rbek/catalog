...
export const SpecialCard = ({ cards, isMobile, modalActiveRef, modalActive }) => {

  const itemClickHandler = React.useCallback((theme, key, approved, index) => {
    let bonus_click = theme.themeId == '3b6159bd-3bf6-4f61-8b5e-f9f5e2878cd7';

    // if (bonus_click) {
    //   return
    // }

    if((!isMobile && bonus_click) || (isMobile && bonus_click)){
      modalActiveRef(true);
    }

    if(approved && (!bonus_click || (isMobile && !bonus_click))) {
      dispatch(clearWords());
  ...
  const renderBoxes = ({ cards, isMobile }) => {
    ...
    if (boxes[boxes.length - 1].length === 1 && !isMobile && boxes.length > 1) {
      let lastElement = boxes[boxes.length - 2].spliace(-1);
    ...
    return boxes.map((box, index) => {
      return (
        ...
        >
          {renderImages({ items: box, isReverse: index % 2, index })}
        </ReactCSSTransitionGroup>
      );
    });
  };

  const renderImages = ({ items, isReverse, index }) => {
    switch (items.length) {
      case 1:
        return <div className={styles.box}>{renderItem(items[0], 0, index)}</div>;
      case 2:
        return (
  ...
  const renderItem = (item, key, index) => {
    if (approwed) {
      return (
          <div onClick={() => itemClickHandler(item, key, approwed, index)} className={styles.box}>
            {renderImage(item)}
          </div>
      );
    } else {
      return (
          <div className={styles.box} onClick={() => itemClickHandler(item, key, approwed, index)}>
            {renderImage(item)}
  ...
  const renderImage = (card) => {
    let { name: title, description } = card;
    // if (card.themeId == '3b6159bd-3bf6-4f61-8b5e-f9f5e2878cd7') {
    //   title = ''
    //   description = ''
    // }
