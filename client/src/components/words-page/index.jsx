...
export const ContentPage = React.memo(() => {
  ...
  const fetchWords = ({limit, page, noImage, subjectId}) => {
    fetch(`${process.env.REACT_APP_API_URL_WORDS}/translation/list?` + new URLSearchParams({
        apiKey: process.env.REACT_APP_WORD_SERVICE_ACCESS_KEY,
        page,
        limit,
        filterBy: `{"subjects": ["${subjectId}"], "lang": "ES"}`
      }))
      .then(response => {
  ...
  const getSearch = (word) => {
    if (word.length >= 3) {
      fetch(`${process.env.REACT_APP_API_URL_WORDS}/translation/all?` + new URLSearchParams({
        apiKey: process.env.REACT_APP_WORD_SERVICE_ACCESS_KEY,
        filterBy: `{"text": "${word}"}`
      }))
      .then(response => {
        ...
        <div className={styles.headerItemWrapper}>
          <div className={styles.breadCrumpWrapper}>
            <Breadcrumb>
              ...
              {currentTheme?.name && (
                <Breadcrumb.Item>Тема: {currentTheme.name}</Breadcrumb.Item>
              )}
            </Breadcrumb>
          </div>
        </div>
